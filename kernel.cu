...
__global__ void fft(float2* data, int n) {
    // 计算线程索引
    ...
    for (int stride = 1; stride <= n / 2; stride *= 2) {
        ...
        if (index < n) {
            ...
            float angle = -2.0f * 3.14159f * tid / (float)n;
            float2 twiddle = make_float2(cos(angle), sin(angle));
            float2 butterfly = temp + twiddle * data[index + stride];
            data[index] = temp + butterfly;
            data[index + stride] = temp - butterfly;
        }
        __syncthreads();
    }
}
...
int main() {
    float2* h_data;  // 主机上的输入信号
    float2* d_data;  // 设备上的输入信号

    // 分配和初始化输入信号
    h_data = (float2*)malloc(N * sizeof(float2));
    for (int i = 0; i < N; i++) {
        h_data[i].x = i;      // 实部
        h_data[i].y = 0.0f;  // 虚部
    }
    ...
    // 执行并行FFT计算
    fft << <gridSize, blockSize >> > (d_data, N);
